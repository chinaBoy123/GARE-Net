...
class VSACoding(nn.Module):
    ...
    def forward(self, imgs, caps, img_lens, cap_lens, return_attn=False):
        max_r,max_w = int(img_lens.max()),int(cap_lens.max())
        imgs = imgs[:,:max_r,:]  # (128, 36, 1024)
        caps = caps[:,:max_w,:]  # (128, 35, 1024)
        sims = get_fgsims(imgs, caps)[:,:,:max_r,:max_w]
        mask = get_fgmask(img_lens,cap_lens)
        ...
        sims = torch.softmax(sims.masked_fill(mask==0, -torch.inf),dim=-1) # Bi x Bt x K x L, masked_fill: 填充元素0为-∞
        ...
        sims = torch.mul(sims.permute(1,0,2,3),imgs).permute(1,0,2,3).sum(dim=-1) \
                    /(torch.norm(sims,p=2,dim=-1,keepdim=False)+EPS) # Bi x Bt x K, (128, 128, 36)

        mask = get_mask(img_lens).permute(0,2,1).repeat(1,cap_lens.size(0),1) # (128, 128, 36), img_lens: (128)
        sims = sims.masked_fill(mask==0, -1) # (128, 128, 36)
        return sims
...
def get_coding(coding_type, **args):  #默认VSACoding
