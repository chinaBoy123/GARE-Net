...
def encode_data(model, data_loader, log_step=10, logging=logger.info):
    ...
    max_n_word = 0
    for i, (images, image_lengths, captions, lengths, boxes, ids) in enumerate(data_loader):
        max_n_word = max(max_n_word, max(lengths))

    for i, data_i in enumerate(data_loader):
        # make sure val logger is used
        images, image_lengths, captions, lengths, boxes, ids = data_i
        model.logger = val_logger

        # compute the embeddings
        img_emb, cap_emb = model.forward_emb(images, captions, lengths, image_lengths=image_lengths, boxes=boxes)
...
def evalrank(model_path, data_path=None, split='dev', fold5=False, save_path=None, cxc=False):
    ...
    if opt.text_enc_type=="bigru":
        ...
        root = osp.expanduser("/home/ubuntu/Students/zhoutao/data/FNE_dataset/google/bert-base-uncased")
...
def compute_sims(img_embs, cap_embs, img_lens, cap_lens, model, shard_size=128):
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            im = torch.from_numpy(img_embs[im_start:im_end]).float().cuda() # cuda->cpu
            # im = torch.from_numpy(img_embs[im_start:im_end]).float()
            s = torch.from_numpy(cap_embs[cap_start:cap_end]).float().cuda() # cuda->cpu
            # s = torch.from_numpy(cap_embs[cap_start:cap_end]).float()
...
def i2t(npts, sims, return_ranks=False, mode='coco'):
    ...
    """
    # caps = []
    # with open('/home/ubuntu/Students/zhoutao/data/coco_precomp/test_caps.txt') as f:
    #     for line in f:
...
def t2i(npts, sims, return_ranks=False, mode='coco'):
    ...
    ids = []
    caps = []
    with open('/home/ubuntu/Students/zhoutao/data/coco_precomp/test_ids.txt') as f:
        for line in f:
    ...
    for index in range(npts):
        if mode == 'coco':
            for i in range(5):
                ...
                if index in [12, 13, 14, 15, 16, 17]:
                    cap_index = 5 * index + i
                    print(caps[cap_index])
                    for j in range(10):
                        print(f'ind={inds[j]}, id={ids[inds[j]]}')
                    print(f'matched_ind={index}, matched_id={ids[index]}')
                    print('\n')
                    break
