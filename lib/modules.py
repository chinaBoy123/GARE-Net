...
def get_fgsims(imgs, caps): # (128, 36, 1024) (128, 30, 1024)
    ...
    sims = sims.reshape(bi, n_r, bc, n_w).permute(0,2,1,3) # (128, 128, 36, 35)
    return sims
...
def get_fgmask(img_lens, cap_lens):
    bi = img_lens.shape[0]
    bc = cap_lens.shape[0]
    max_r = int(img_lens.max()) # 36
    max_w = int(cap_lens.max()) # 35
    ...
    mask_i = mask_i.reshape(bi*max_r,1) # (1, 4480)
    ...
    mask_c = mask_c.reshape(bc*max_w,1).t()  # (1, 4480) 不是1就是0

    mask = torch.matmul(mask_i,mask_c).reshape(bi, max_r, bc, max_w).permute(0,2,1,3) # (128, 128, 36, 35)
    return mask
